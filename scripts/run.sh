...
# Run the main rule-generation flow with the provided paths.
run_main_operations() {
    project_path=${1:-"."}
    output_root=${2:-""}
...
# Parse the CLI command and options, then return source/output values.
parse_cli() {
    if [ $# -eq 0 ]; then
        print_usage
        exit 0
...
parse_cli "$@"
run_main_operations "$project_path" "$output_root"
